...
Enemy intent (BUILD 141, F34 — supersedes the flat single-roll description this paragraph used to give): every enemy now acts from its own `pattern`, a repeating list of 1-4 intents — {kind:'attack',min,max}, {kind:'charge',release,breakAt}, or {kind:'afflict',stacks} — fixed onto the live enemy by beginFightFromSlot() from the entering slot's own static pattern (buildAct()), same as hp/intentMin/intentMax. Every enemy buildAct() builds today still carries a single-entry attack pattern using that same intentMin/intentMax band (normal 6-18, elite 10-18, boss 10-20 — act 1 values; see ACTS for other acts), so play is unchanged from before this build — the multi-intent patterns are for BUILD 142's designed enemies. advanceEnemyIntentForRound() (pipeline.js) fixes and shows this round's entry at the very top of START_OF_TURN (before poison ticks or block clears, same spot the old flat roll sat); advanceEnemyPattern() moves the pointer on in ENEMY_ACT_PHASE once a round resolves — a charge only advances after its own release round. A Charge takes two rounds: a no-damage wind-up (windupStartHp recorded before that round's own poison tick) then a release; if the enemy's own HP falls by breakAt or more during the wind-up (poison ticks count), the release deals nothing (chargeBroken). An Afflict round applies its stacks of poison to the player, no damage. getIncomingIntentDamage() (pipeline.js) is the one shared reader for "how much is this round's intent about to deal" — Interdict (cards-mods.js) reads it instead of a flat intent number. The enemy's own Nat 1 cancels whatever intent is live this round, including cancelling a wind-up's entire charge (the pattern skips straight past it). Dev-only Set Next Intent (devSetNextIntent(), dev-tools.js) overrides the next round's pattern-derived entry exactly once. The die still only determines whether a buff or Nat triggers — it never modifies intent (Wrath, BUILD 141 below, is the one exception: it adds to a future round's already-fixed Attack roll, never changes a round's value after it's shown).
...
ENEMY DICE OF ANY SIZE, WRATH, DRAIN, SEAL (BUILD 141, item C, F35 — BASE for BUILD 142): an enemy's die size is no longer ELITE/BOSS-only — GAME_CONFIG.DIE_SIZE gained NORMAL (6), and GAME_CONFIG.DEV_TEST_DIE_SIZES ([6, 12, 20]) backs a dev-only Test Die control (#devTestDie, devSetTestDie(size, buffId), dev-tools.js) that gives the current enemy a die of any of those sizes with one buff on every face, this fight only. renderDieList() (rendering.js) needed no change to draw a 6- or 12-row die — it already builds one row per element of whatever faces array it's handed. buildEnemyDieFromSpec(spec) (pipeline.js) is a new generic builder from a {sizeKey, faces: {faceNumber: buffId}, nats} shape, for a future designed enemy (BUILD 142) to use instead of buildEnemyDieFaces() — today's elite and boss dice are untouched, still built exactly as ENEMY DIE PER TYPE above describes. Three new buffs, dispatched through the existing enemy_buff_dispatch listener (cards-mods.js) alongside enemy_buff_poison: Wrath (enemy_buff_wrath, GAME_CONFIG.ENEMY_WRATH_AMOUNT per trigger) queues into gameState.enemy.wrathPending, which moves into gameState.enemy.wrath (added to every later Attack roll) at the next START_OF_TURN — a round's already-shown Attack value never changes retroactively. Drain (enemy_buff_drain) queues 1 onto gameState.player.drainNextRound, consumed by the next START_OF_TURN's soul reset (soul comes in that much under maxSoul, floored at 0). Seal (enemy_buff_seal) queues the player's own heaviest loaded face, excluding faces 1/20 and any face already Sealed this round, ties going to the lowest-numbered face (pickHeaviestLoadedFaceForSeal(), pipeline.js — same tie rule Magnificat's own heaviest-face search uses) — the queue (gameState.player.sealNextRound) moves into gameState.turn.sealedFaces at the next START_OF_TURN, where it starts actually counting. All three buffs fire under an enemy Nat 20 sweep exactly like poison does, since that sweep already dispatches ENEMY_BUFF_TRIGGER generically for every loaded face — no buff-specific code was needed there. See LOADED-FACE RULE below for what a Sealed face actually changes. On-screen: enemy panel gains a WRATH +N stat line (shown only above 0); player panel gains a DRAIN −N SOUL marker (shown only when queued); a Sealed die row dims (existing --muted colour, no new palette) and shows SEALED, or SEALED NEXT ROUND for a face merely queued — all plain text, all with hover text, per the BUILD 143 UI-pass note.

LOADED-FACE RULE / SEAL (BUILD 141): isFaceSealed(faceNumber) (pipeline.js) — checks gameState.turn.sealedFaces — is the one new shared "is this face sealed" check. There was no single existing "is this face loaded" helper for it to slot into (confirmed by this build's own code audit — see MULTI-MOD FACES' own listing of every site that decides loadedness): resolvePlayerRoll()'s dispatch, isBoundFace() (which the Bound scan and the Nat 20 sweep both already route through, so a Sealed Bound face is skipped by both for free), triggerFaceOutsideRoll() (so Reverberation on a Sealed face gives the plain 2-block blank), onNatTwenty()'s own sweep filter, and the loaded-face filters inside Gradual, Threnody, Canticle, Magnificat, and Herald were each changed individually to also require `!isFaceSealed(...)`. Covenant/Tabernacle (which read rolledFaceWeight directly, never checking loadedness) and Tenet (which reads a face's trigger-count history, not its current loaded state) needed no change. Faces 1 and 20 are never Sealed. Sealed is round-scoped (gameState.turn.sealedFaces, cleared at START_OF_TURN like every other round-scoped roll flag) and layered on top of, not instead of, Bound's own fight-scoped grant mechanism (BOUND ENGINE) — a face can be both Bound and Sealed at once, in which case Sealed wins for that round (isBoundFace() checks isFaceSealed() first).
