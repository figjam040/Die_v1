...
Twenty-seven mods, all in config.mods. Consecrate is excluded from the reward pool because it's already loaded on the Ordained's starting die (CLASS OBJECT STRUCTURE); the other twenty-six are reward-eligible, each with a tier ('basic'/'uncommon'/'rare') and a tags list. See MOD_DESCRIPTION (rendering.js) for the live, plain-text description of every mod — the source a player reads from.
